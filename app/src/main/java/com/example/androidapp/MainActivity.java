...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        //해당화면이 실행되면 처음으로 실행될 메서드
        //여깅[ 코드를 넣어놓으면 화면이 처음 실행될 때 동작할 코드가 됨
        super.onCreate(savedInstanceState); //AppCompatActivity 부모 클래스의 onCreate 메서드를 실행
        setContentView(R.layout.activity_main);     //res폴더 안에 layout 폴더 안에 activity_main.xml화면을 실행

        //화면 리소스와 자바변수를 연동
        button1 = findViewById(R.id.button1);
        text1 = findViewById(R.id.text1);
        edit1 = findViewById(R.id.edit1);
        // 버튼 이벤트 추가 (버튼이 눌렸을 때 발생하는 코드 추가)
